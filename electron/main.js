...
// ─── yt-dlp ──────────────────────────────────────────────────────────────────
function ytdlpBin() {
  // Check if user wants to use system yt-dlp
  const settings = readJSON(FILES.settings, DEFAULT_SETTINGS);
  if (settings.useSystemYtdlp) {
    try {
    ...
    const macPaths = [
      ...
      '/Users/' + require('os').userInfo().username + '/Library/Python/3.9/bin/yt-dlp',
      '/Users/' + require('os').userInfo().username + '/Library/Python/3.10/bin/yt-dlp',
      '/Users/' + require('os').userInfo().username + '/Library/Python/3.11/bin/yt-dlp',
      '/Users/' + require('os').userInfo().username + '/.local/bin/yt-dlp',
    ];
